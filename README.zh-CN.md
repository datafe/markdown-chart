...
resolver 返回 `{ dimensions?, source }`。ECharts 使用这些行数据绘图，并把它们回传给 core 的 `Chart / Data` 视图，因此无需在 Markdown 中重复内联数据也能查看数据表格。
