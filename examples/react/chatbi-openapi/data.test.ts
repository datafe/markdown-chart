...
describe('ChatBI OpenAPI ArtifactContent integration', () => {
  ...
  it('rejects missing, ambiguous, and non-string ArtifactContent', async () => {
    const missing = createChatBIArtifactContentResolver({
      sessionId: 'session-1',
      fetch: vi.fn().mockResolvedValue(rpcResult({ Artifacts: [], NextToken: null })) as unknown as typeof fetch,
    });
    await expect(missing(request())).rejects.toThrow('was not found');

    const ambiguous = createChatBIArtifactContentResolver({
      sessionId: 'session-1',
      fetch: vi.fn().mockResolvedValue(rpcResult({
        NextToken: null,
        Artifacts: [
          { ArtifactName: 'chatbi_query_42.csv', ArtifactPath: 'a.csv' },
          { ArtifactName: 'chatbi_query_42.csv', ArtifactPath: 'b.csv' },
        ],
      })) as unknown as typeof fetch,
    });
    await expect(ambiguous(request())).rejects.toThrow('ambiguous (2 matches)');
